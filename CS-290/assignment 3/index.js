 ...
 */
 function postGenerator(ItemName, URL, Price, City,Condition){
    ...
    var filters = document.getElementById('filter-city');
    var counter = 0;
    for(var i =0; i < filters.length;i++){
      if(filters[i].text.toLowerCase() == City.toLowerCase()){
        counter = 1;
        break;
      }
      else{
    ...
    if(counter == 0){
      var tempfilter = document.createElement('option');
      var newcity = document.createTextNode(City);
      tempfilter.appendChild(newcity);
      filters.appendChild(tempfilter);
    }
 }
/////////////////////////////////////////////////////////////////////////////////////////////////////
function clearinig(){
...
function update(){
  ...
  if(CityFilter != ''){
    ...
    while(i--){
      ...
      name = temp.replace(/[()]/g,'');
      if(name.toLowerCase() != CityFilter.toLowerCase()){
        Posts[i].remove();
      }
    }
  }
