 ...
 */
// This code was snippeted from https://www.geeksforgeeks.org/queue-set-1introduction-and-array-implementation/
...
{
  // assert(da);
  // if (!(da->start <= da->size && idx >= 0))
  // {
  //   printf("start == %d  ====== size== %d ======= idx== %d", da->start, da->size, idx);
  // }
  // assert(da->start <= da->size && idx >= 0);
...
void set_start(struct dynarray *da, int x)
{
  da->start = x;
}
...
void remove_queue(struct dynarray *da)
{
